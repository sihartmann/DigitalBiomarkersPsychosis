...
* 1.2.3 with GUI
    - Added interviewer analysis option -> if activated, interviewer audio and video will be analysed too
    - OpenFace will only extract gaze, pose, 3D landmarks, and action units
    - Log file is stored in directory with all participant folders
...
- Create a folder for every participant. The name of this folder will be the subject's ID. Each folder must contain the following files, e.g. downloaded from HIPAA zoom.
    ...
    - A file including ‘gvo’ and ending on ‘.mp4’ indicating the recorded video, e.g. when downloaded from HIPAA Zoom [date and time of the interview]_Recording_gvo_1280x720.mp4
    - If interviewer analysis and no video cropping activated: A file including ‘interviewer and ending on ‘.mp4’ indicating the recorded interviewer video, e.g. [date and time of the interview]_Recording_interviewer.mp4
...
- Interviewer: Analyse interviewer data in addition to participant data
...
## OUTPUT
-	**logs**: Contains logs generated by ffmpeg, Whisper and OpenFace (depending on the ‘Verbosity’ that was selected). If there are any errors, the log files are a good starting point for troubleshooting. Main log file called DigBio.log is stored in the directory with all the participant folders.
...
If interviewer analysis is activated, most files are also generated for the interviewer indicated by ‘interviewer’ in the filename.
